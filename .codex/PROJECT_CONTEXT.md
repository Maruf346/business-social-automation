...
- First Telegram review/assignment/offer cards include a `Reference Image(s) for Request #REQUEST_ID:` section below Summary. When images exist, the section says they are attached below and the bot sends one Telegram photo for a single image or one Telegram media-group album for multiple images; when none exist, it shows `None`.
- A scheduled TA/TC Google-only external appointment can be reassigned to Hoss/Nina. The old external artist Google Calendar event is kept until Hoss/Nina successfully reschedules the request with a vCita-backed service, then the old external artist event is released.
...
- Temporary Outlook testing gate is active in `site_config/settings.py`: `OUTLOOK_REQUIRE_TEST_SUBJECT=True` means only Outlook emails whose subject contains `test` are processed; all other Outlook emails are skipped before lead/message creation, AI, Telegram, or replies. Set it to `False` for production launch.
...
- When the external artist declines, the offer card buttons are removed, the group is notified, and the intake remains available for reassignment or manual handling. Hoss/Nina can run `/reassign REQUEST_ID` to receive a short artist-selection card; once a new artist is selected, that short card is edited and its buttons are removed. If the request is already scheduled in vCita, reassignment is allowed only between approver artists such as Hoss/Nina and vCita is not changed because they share the same vCita staff UID. If the request is already scheduled as Google-only TA/TC, reassignment is allowed only to another external artist after their Google Calendar availability passes; the new artist event is synced and the old artist event is released.
