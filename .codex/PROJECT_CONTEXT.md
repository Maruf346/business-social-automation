...
- First Telegram review/assignment/offer cards include a `Reference Image` section below Summary. When images exist, the section says they are attached below and the bot sends one Telegram photo for a single image or one Telegram media-group album for multiple images; when none exist, it shows `None`.
