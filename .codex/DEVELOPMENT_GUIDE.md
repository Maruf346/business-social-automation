...
- First Telegram group review cards, internal artist assignment cards, and external artist offer cards show `Reference Image(s) for Request #REQUEST_ID:` below Summary. The text avoids raw URLs; actual stored incoming image references are sent immediately after the card as one Telegram photo for a single image or one Telegram media-group album for multiple images.
...
- Hoss/Nina can use `/reassign REQUEST_ID` to get a short artist-selection card. Selecting an artist edits that short card, removes its buttons, and either assigns an approver artist directly or sends an external artist offer. Scheduled vCita requests can only be reassigned between approver artists; scheduled Google-only TA/TC requests can only be reassigned to another external artist after Google Calendar availability is checked and the old artist calendar event is released.
